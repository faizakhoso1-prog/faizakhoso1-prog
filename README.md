...
I specialize in **High-Speed Data Acquisition (DAQ) systems, FPGA instrumentation, and hardware resilience**. My academic work focuses on bridging the gap between nuclear physics instrumentation and hardware-level cybersecurity to design reliable electronics for harsh, radiation-heavy environments.
...
* Developed a fully synchronous **Triple Modular Redundancy (TMR)** control core paired with an ultrafast combinational majority voting loop.
* Eliminated transient hardware routing glitches at state transition boundaries using synchronous edge-triggered registers.
* Integrated an autonomous Fault Detection and Isolation (FDI) telemetry engine designed to log Single Event Upsets (SEUs) directly to SOC security monitoring dashboards.
...
* **Core Circuit Competency:** High-Speed Time-to-Digital Converters (TDC), Triggering Logic, AXI4 Interconnects, Hardware-Software Co-Design.
* **Analog Front-End:** Pulse shaping and conditioning, high-speed low-noise preamplifiers (1ns rise time), advanced LTspice transient analysis.
